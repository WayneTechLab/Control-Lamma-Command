...
> You can **paste the Web `firebaseConfig` block** directly, or point the prompt
> at the **`GoogleService-Info.plist` / `google-services.json` file path** and it
> will be parsed automatically (manual field entry is always available as a
> fallback).
