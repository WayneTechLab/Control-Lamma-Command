...
bash .SYSTEMX/scripts/bootstrap.sh --check         # verify only
```
...
└── starter/                      ← ⭐ the runnable app you copy to start a new project
    ├── src/  (Home, About, Features, Contact, 404 + Navbar/Footer layout)
...
| Build / dev server | Vite 7 | No — core of the template |
...
npm run build
# (optional) firebase use --add && firebase deploy --only hosting
