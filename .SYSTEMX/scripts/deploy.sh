...
# Defaults
DO_TYPECHECK=1; DO_LINT=1; DO_FIX=0; DO_TESTS=1; DO_BUILD=1
DO_PUSH=1; DO_DEPLOY=1; OPEN_BROWSER=0; PRECHECK_ONLY=0
...
log "=== WebApp Stack G1 deploy (run $RUN_ID) ==="
...
if [[ $DO_DEPLOY -eq 1 ]]; then
  ...
  FB_TARGETS=$(node -e "
    ...
    const t=[];
    if(j.hosting) t.push('hosting');
    if(j.firestore?.rules) t.push('firestore:rules');
    if(j.firestore?.indexes && fs.existsSync('firestore.indexes.json')) t.push('firestore:indexes');
    if(j.storage) t.push('storage');
    if(j.functions || fs.existsSync('functions/package.json')) t.push('functions');
